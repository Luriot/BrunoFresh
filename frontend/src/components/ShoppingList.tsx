...
export function ShoppingList({ data }: Props) {
  if (!data) {
    return <p className="text-sm text-gray-600">Generate list to see aggregated groceries.</p>;
  }
    ...
    <div className="space-y-4">
      {Object.entries(data.grouped).map(([category, items]) => (
        <section key={category} className="rounded-2xl border border-orange-200 bg-white p-4">
          <h4 className="mb-2 font-heading text-lg font-semibold text-ink">{category}</h4>
      ...
      {data.needs_review.length > 0 && (
        <section className="rounded-2xl border border-amber-300 bg-amber-50 p-4">
          <h4 className="mb-2 font-heading text-lg font-semibold text-amber-900">Needs Review</h4>
